...
export const getMe = async (req: Request, res: Response) => {
  try {
    const user = await Users.findOne({ accessToken: { $in: req.cookies["token-ex-ts-mongoose"] } }).select([
      "-__v",
      "-password",
      "-accessToken",
    ]);
    ...
    if (!user) {
      res.status(403).json({ error: `forbidden, accessToken tidak valid` });
      return;
    }
...
export const updateMe = async (req: Request, res: Response) => {
  try {
    ...
    if (!existingDataToken) {
      res.status(403).json({ error: `forbidden: token tidak valid` });
      return;
    }

    if (existingDataToken.role !== "admin" && req.body.role === "admin") {
      res.status(400).json({ error: `user cannot be an admin without admin permission` });
      return;
    }

    if (existingDataToken.email === "ahmad@gmail.com" && req.body.role === "user") {
      res.status(400).json({ error: `You are primary admin you cannot be the user` });
      return;
    }
    ...
    if (password) {
      if (password !== confirmPassword) {
        res.status(400).json({ error: `confirm password wrong` });
        return;
      }
...
export const deleteMe = async (req: Request, res: Response) => {
  try {
    const existingDataToken: IUser | null = await Users.findOne({
      accessToken: { $in: req.cookies.accessTokenV1 },
    });
    if (!existingDataToken) {
      res.status(403).json({ error: `forbidden: token tidak valid` });
    ...
    if (existingDataToken.role === "admin") {
      res.status(400).json({ error: `role admin cannot be deleted, change role first` });
      return;
    }
